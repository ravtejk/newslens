...
| `ANTHROPIC_API_KEY` | Yes | The Claude API lane (Claude Haiku 4.5): the ranking, editorial-tighten, and TTS-script seats (B2 depth flip, 2026-07-16). Standard key, default permissions; **set a hard monthly spend cap in the Anthropic console** — Haiku is $1.00/$5.00 per MTok in/out, pennies per run. A normal `newslens generate` cannot run without it (the doctor validates it with a harmless read-only call and FAILs the run when it's missing). Rollback: flip these three seats back to GPT-4o/api in `llm.py` — one diff. |
