...
| `newslens profile delete <name> [--confirm <name>]` | **NL-132-B.** Permanently remove one reader's whole world — database, corpus, artifacts, spend ledger, `memory.md`, `sources.yaml`. **Dry run by default:** without `--confirm` it prints what would go (file/directory count, bytes, that profile's own status line) and changes nothing; `--confirm` must be that profile's slug typed back exactly. Refuses the founder's world **structurally, on resolved paths rather than a name comparison**. Seven guards, each with its own test: the slug `default`; a symlink at `profiles/` **itself**; a symlink at `profiles/<slug>`; a name that is not a directory; the one-level-below shape; **containment** — anything that resolves inside this checkout must resolve inside this checkout's own `profiles/`, whatever anchor or `NEWSLENS_DATA_DIR` was in force; and any directory that is or contains one of the checkout's guarded locations. **What that earns:** no anchor manipulation can aim this verb at your `data/` (including every dated subtree under it), the checkout root, `src/`, `tests/`, or `profiles/` itself. It claims nothing about a world *outside* this checkout — there the anchor is your own declaration of which world you mean. After removing the tree it **re-stats it**: if the directory is back — a live serve on that profile re-creates it on its next request — it says so and exits non-zero instead of printing "(gone)". Replaces the old teardown advice ("delete that directory by hand"), which was an `rm -rf` typed repeatedly beside your own state. |
| `newslens profile refresh-catalog <name> [--apply] [--skip "Name"]` | **The catalog-refresh verb.** A profile's `sources.yaml` is a *copy* of the org template taken once at `create` and frozen there, so a reader provisioned before a feed slate landed never sees it — `fresh1` (created 2026-08-02, one day before the NL-135 slate) ranked 385 general-news items against health interests it had no health feeds to serve. This is how that reader adopts the update: **by typing it.** Nothing calls it automatically — no generate hook, no doctor autofix, no migration; that file is yours. **Dry run by default:** it prints the whole delta and changes nothing until `--apply`. **Add-only, verified rather than promised:** it appends entries whose *name* your file lacks, copied verbatim from the org catalog with their own comments, and never rewrites, re-enables, disables, re-URLs, re-tiers or removes anything already there, and never touches `interests:` or `settings:`. Where the org now disagrees about a source you already have (the org disabled CNN on 2026-08-07; a profile frozen before that still has it enabled) the disagreement is **reported and left alone** — your line wins. Before any write lands, the merged text is parsed in a temp file and compared field-by-field against your current one; a single changed pre-existing source, a touched interest, or unparseable YAML aborts the write with the original byte-identical. **Merge with provenance:** a comment ledger in your own file records what you have been offered, so a feed you adopt and later delete is never offered back, and `--skip` is remembered as a decline. Refuses the founder's own catalog by name *and* structurally, on resolved paths — a link named anything else still resolves to his file, and a catalog that resolves outside its own profile root is refused too. |
| `scripts/reader-serve <name> [--live] [--port N]`<br>`scripts/reader-serve --create <name>` | **NL-132-B — the review door.** Serves any *non-founder* profile with the **$0 scrub structural by default**: the `PERPLEXITY_API_KEY=` present-but-empty form is applied to the process and then **measured in a child after `load_env`** (the same `enforce_zero_dollar` both persona doors use), so no metered Sonar call can fire from the served page's Generate button — and you never have to remember the shell prefix (`env -u` is the spelling that *spends*; see `personas.zero_dollar_env`). `--live` is the explicit opt-in to your real key and says so out loud, with the key's length, never the key. `--create` mints the world and serves it in one command. The port is **probed, not assumed** — never 8484, never one something is already holding. At exit it prints that profile's own ledger: what the session charged (real money) and shadowed (the API-equivalent the cap guards). Your own world is refused here: it is served by `newslens serve` on 8484. |
...
| `newslens memory-baseline (--thread-id N \| --all) [--date YYYY-MM-DD]` | NL-77: writes the cold-start **backgrounder** (entry-zero baseline — "How we got here") for followed threads with an EMPTY ledger. One analyst-model call each (GPT-4o pointed backwards, ~$0.01–0.02, marked `external-synthesis`, cite currency `(baseline, <date>)`), validated (rejects bare continuity diction → an honest `failed` row, never fabricated), spend durable on the `thread_baselines` row. Cap pre-checked; single selector enforced; refuses when nothing awaits. **The `--all` retroactive sweep is a principal checkpoint** (his 18-thread sweep ran 2026-07-16; since migration 0027 a thread delete takes its baselines with it, so deletes no longer need to precede a sweep). |
...
|---|---|---|
| `OPENAI_API_KEY` | **Only for openai TTS** | **No text seat spends this key today.** `synthesis` is the last gpt-4o seat on the roster and it has no live call site (same fact as the `generate` row above); writer, analyst, editor, script, rank, state and the follow-altitude resolver are all Claude seats on the subscription lane. The one live caller is **audio, and only if you ask for it**: NL-96 (the $0-run law, 2026-07-25) made Kokoro-82M local the default voice, so a default install charges this key nothing and does not need it. gpt-4o-mini-tts on this same key stays fully built and is the principal's ear-test pick (ruling 2026-07-06 — a voice ruling that stands); it costs ~$0.015/min, ~+$0.07/run once pinned with `settings.tts_engine: openai`. Standard key, default permissions; set a hard spend cap in the OpenAI dashboard. *(This row read "Required: Yes — the narrative (writer), analyst, and synthesis seats" until NL-155, 2026-08-14. Writer and analyst left for the Claude lane at B4/item C 2026-07-17 and state followed at ENG-M0 2026-08-06; the row outlived all three moves and kept demanding a key the default path never uses.)* |
| `ANTHROPIC_API_KEY` | Fall-over only | The Claude **API** lane credential — the registered fall-over for every anthropic seat (Opus 4.8 for writer/analyst/editor/script/state, Sonnet 5 for rank and the follow-altitude resolver). **B3 (2026-07-16) flipped the ranking, editorial-tighten, and TTS-script seats to the `claude -p` SUBSCRIPTION lane by default** — they ride your Claude subscription (usd_charged $0.00; usd_shadow still ledgered), so the default path needs the CLI installed + logged in, NOT this key (see the Claude CLI row + SETUP.md §2c). The key is required only when a seat runs the API lane: you pin `NEWSLENS_LANE_<SEAT>=api`, or you arm `NEWSLENS_LANE_FALLBACK=api`. **Set a hard monthly cap in the Anthropic console** if you use it. Rollback: a seat's registered fall-over is **its own row with `lane="api"`** — pin it with `NEWSLENS_LANE_<SEAT>=api`, no code edit. Moving a seat's MODEL has no registered alternative by design (NL-147) and goes through the principal. |
| Claude CLI (`claude`) | Yes (subscription lane) | The subscription lane's transport: **all seven Claude seats** — `rank`/`editor`/`script`/`writer`/`analyst`/`state`/`follow_altitude` — default to `claude -p` against your logged-in CLI. (This named only `rank`/`editor`/`script` until NL-155, 2026-08-14: the B3 trio, before item C and ENG-M0 moved the other four onto the same lane. It is the CLI, not `ANTHROPIC_API_KEY`, that a default install must have.) Resolution: `NEWSLENS_CLAUDE_BIN` → `PATH` → `~/.local/bin/claude` (the doctor reports which resolved and the version). Grant it by installing the CLI and running `claude` once to log in — NewsLens never handles your credentials; the subprocess strips `ANTHROPIC_API_KEY`, disables all tools + CLAUDE.md/skills/plugins/hooks/MCP, and runs in an empty scratch dir. A missing/unauthed CLI FAILs the run naming the fix (never a silent API call). |
...
| `GENERATE_HOUR_LOCAL` | Optional, default 6 | The local hour (0-23) a SCHEDULED run fires at — the FALLBACK layer since NL-152. **Settings › Generation time WINS over this when set** (it writes `settings.generate_hour` in sources.yaml); the doctor prints one extra line naming which layer decided whenever that override is live. Dormant 2026-07-03 -> 2026-08-13; NL-146 woke it — `newslens schedule plist` bakes the RESOLVED hour into the launchd agent (SETUP.md §5), and the doctor / `schedule status` warn when an installed agent's hour and the resolved hour disagree. On-demand `generate` ignores it. A set-but-invalid value still fails the doctor (a typo'd .env is a config error). |
