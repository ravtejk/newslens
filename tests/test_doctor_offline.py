...
SCRATCH_TABLES_LINE = (
    ...
    "analysis_briefs, analysis_retrieval, briefings, briefings_history, briefings_pending, concept_explanations, consumption_events, entities, follow_altitude_events, follow_settle_events, memory, memory_tombstones, ranking_runs, source_items, sync_state"
)
...
def test_keyless_template_run_exits_1_with_fix_hints_and_zero_network(
    ...
    assert (f"default {config.DEFAULT_BUDGET_CAP_USD_PER_RUN:.2f}" in out)
...
def test_doctor_flags_a_db_behind_on_migrations(tmp_paths, no_network, capsys):
    ...
    assert code == 1
    assert (
        "behind by 25 migration(s) (0001_initial_schema.sql, "
