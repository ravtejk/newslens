...
| `test_doctor_m2.py` | `GENERATE_HOUR_LOCAL` doctor wording — dormant until NL-146 woke it 2026-08-13, garbage still fails in both eras — tier-aware sources section, keyless-never-calls-APIs invariant, unreadable `.env`/ping-file pins |
| `test_nl146_schedule.py` | NL-146 scheduled generation — same-day idempotence (readability, not row existence), kill switch incl. re-read during backoff, the retry fork (only a systemic fetch failure retries), the session budget that stops an unattended morning outspending an attended run, atomic publish through a mid-pipeline kill, trigger provenance on both log arms + the reports screen, launchd plist render/refusal, doctor status honesty |
| `test_nl146_schedule_qa.py` | NL-146 QA acceptance contracts (5), born red on the build tree — F-1: a fire-decision line must never become "the run entry for the date" in `_log_entry_for` (the quiet note survives the ladder's own fire line; a scheduled success keeps its structured stories); plus the unattended session never charging past one run's cap, and a body-less row never listed as an edition in the archive |
| `test_nl146_fixloop1.py` | NL-146 fix loop 1 — the cross-process generation slot (`data/RUN_IN_FLIGHT`): the POST door and the scheduled ladder each decline while the other generates, Today says "being generated" instead of "nothing yet", staleness bounded by pid liveness first and age second, the slot released for the backoff and never held across the ladder; plus the archive's half of "a row is not an edition", the doctor's per-line severity + single read, the wired ladder clock, and fire lines never counted as malformed records. Gate fixlet: the failed fire records the attempts that RAN (both directions, not the ceiling), the quiet note never claims "nothing was charged" against its own `total_usd` (unreadable makes no claim), and the status readout leaves a DEAD marker on disk — the case a live-marker pin structurally cannot reach |
