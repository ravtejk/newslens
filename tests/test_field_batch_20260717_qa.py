...
def test_timeout_sub_map_is_pinned_exactly():
    ...
    # 2026-07-26, TWO MOVES in sequence and the map records the end state.
    # (i) The watchdog re-tune raised all four mechanical Haiku seats 300 -> 600
    # for the TAXED present. (ii) The thinking-seam flip then suppressed
    # thinking on editor/script/state and re-tuned their walls back DOWN to
    # >=3x their measured off-arm ceilings (editor 54.5s -> 180, script 28.5s ->
    # 120, state 6.0s -> 60). rank keeps 600 because rank is NOT flipped — it is
